...
public class DatabaseWrapperTest extends AndroidTestCase {
	...
	private void assertEventCountInDatabase(int rowCount) {
		Assert.assertEquals(rowCount, DatabaseWrapper.getNumberOfRowsInTable(Database.EVENTS_TABLE_NAME));
	}

	public void testGetNumberOfRowsInTableForBadTablenames() {
		Assert.assertEquals(-1, DatabaseWrapper.getNumberOfRowsInTable(null));
		Assert.assertEquals(-1, DatabaseWrapper.getNumberOfRowsInTable("COBRA COMMANDER WUZ HERE"));
	}
	...
	public void testBadUpdate() {
		...
		assertNotNull(uri);
		values.put(BaseColumns._ID, "THIS AIN'T NO ID");
		Assert.assertEquals(-1, DatabaseWrapper.update(uri, values, null, null));
	}
}
